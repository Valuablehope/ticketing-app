  ...
  showToast(message, type = 'info', duration = 5000) {
    ...
    toast.innerHTML = `
      <div class="toast-icon">
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor">${iconMap[type] || iconMap.info}</svg>
      </div>
    ...
    modalBody.innerHTML = `
      <div class="ticket-detail-grid">
        <div class="detail-section">
          ...
          <div class="detail-item"><label>Status</label><p><span class="status-badge status-${ticket.status?.toLowerCase().replace(' ', '-')}">${ticket.status}</span></p></div>
          <div class="detail-item"><label>Priority</label><p><span class="priority-badge priority-${ticket.priority?.toLowerCase()}">${ticket.priority}</span></p></div>
        </div>
        ...
        <div class="detail-section">
          ...
          <div class="detail-item"><label>Submitter</label><p>${ticket.submitter_name}<br><small>${ticket.submitter_email}</small></p></div>
          <div class="detail-item"><label>Created</label><p>${new Date(ticket.created_at).toLocaleString()}</p></div>
        </div>
      </div>
    `;
  ...
  setupModalEventListeners() {
    ...
    if (ticketModal) {
      ticketModal.addEventListener('click', (e) => {
        if (e.target.id === 'ticket-modal') this.closeTicketModal();
      });
    }
  }
